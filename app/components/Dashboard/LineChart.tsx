...
export const LineChart = ({ data, dateArray, showLabel }: LineChartProps) => {
  if (!data || data.length === 0) {
    return <div className='text-gray text-center'>No data</div>;
  }

  if (!dateArray || dateArray.length === 0) {
    return (
      <div className='text-center text-gray-500'>No date periods provided</div>
    );
  }
  ...
  if (!hasValidData) {
    return (
      <div className='text-center text-gray-500'>No valid data to display</div>
    );
  }
  ...
  return (
    <ChartContainer config={chartConfig} className='size-full h-[240px]'>
      <LineChartComponent
        className='text-sm'
        data={chartData}
        margin={{
      ...
      >
        ...
        <XAxis
          ...
          label={
            showLabel
              ? { value: 'Period', position: 'insideBottom', offset: -10 }
              : undefined
          }
        />
        <YAxis
          label={
            showLabel
              ? { value: 'Amount ($)', angle: -90, position: 'insideLeft' }
              : undefined
          }
